...
public:
    ...
    static std::shared_ptr<SpriteAtlas> createSingleSprite(const std::shared_ptr<Texture>& texture, std::string_view name = "sprite", glm::vec2 pivot = { 0.5f, 0.5f }, glm::ivec2 pos = { 0, 0 }, glm::ivec2 size = { 0, 0 });
    Sprite get(std::string_view name); // Return a copy of a Sprite object.
    std::vector<std::string> getNames();
    [[nodiscard]] const std::string& getAtlasName() const { return m_atlasName; };
    [[nodiscard]] inline const std::shared_ptr<Texture>& getTexture() const { return m_texture; }; // return sprite texture
