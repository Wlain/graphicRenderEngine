...
{
    int maxSceneLights = Renderer::s_instance->s_maxSceneLights;
    if (worldLights == nullptr)
    {
        ...
        glUniform4fv(m_uniformLocationLightPosType, Renderer::s_maxSceneLights, noLight.data());
        glUniform4fv(m_uniformLocationLightColorRange, Renderer::s_maxSceneLights, noLight.data());
        return false;
    }
...
void Shader::updateUniformsAndAttributes()
{
    ...
    for (int i = 0; i < uniformCount; i++)
    {
        ...
        else
        {
            ...
            {
                if (uniformType == UniformType::Vec4 && size == Renderer::s_instance->s_maxSceneLights)
                {
...
{
    std::stringstream ss;
    ss << "#define SI_LIGHTS " << Renderer::s_instance->s_maxSceneLights << "\n";
    // add shader type
    switch (shaderType)
    {
    case GL_FRAGMENT_SHADER:
