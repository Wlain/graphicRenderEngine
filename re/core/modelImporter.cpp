...
glm::vec4 toVec4(std::vector<std::string>& tokens)
{
...
glm::vec3 toVec3(std::vector<std::string>& tokens)
{
    glm::vec3 res{ 0, 0, 1 };
    for (int i = 0; i < 3; i++)
    {
...
ObjFace toObjFace(std::vector<std::string>& tokens)
{
    ObjFace res;
...
void parseMaterialLib(std::string& materialLib, std::vector<ObjMaterial>& materials)
{
    ...
    std::stringstream ss{ materialLib };

    while (ss.good())
    {
        ss.getline(buffer, bufferSize);
        std::stringstream likeTokensizer{ buffer };
...
std::shared_ptr<Material> createMaterial(std::string materialName, const std::vector<ObjMaterial>& matVector, std::string path)
{
    if (matVector.empty())
    {
    ...
    for (auto& map : foundMat->textureMaps)
    {
        ...
        {
            material->setTexture(Texture::create().withFile(fixPath(path + map.filename)).build());
            name += " " + map.filename;
        }
    }
...
std::shared_ptr<Mesh> ModelImporter::importObj(const std::filesystem::path& path, std::vector<std::shared_ptr<Material>>& outModelMaterials)
{
    std::string file = getFileContents(path);
    ...
    while (ss.good())
    {
        ...
        { // vertex position
            glm::vec3 vertexPosition = toVec3(tokens);
            vertexPositions.push_back(vertexPosition);
        }
        else if (tokens[0] == "vt")
        { // vertex texture coordinates
            glm::vec4 textureCoord = toVec4(tokens);
            textureCoords.push_back(textureCoord);
        }
        else if (tokens[0] == "vn")
        { // vertex normal
            glm::vec3 normal = toVec3(tokens);
    ...
    for (auto& face : faces)
    {
        ...
        for (int i = 2; i < face.size(); i++)
        {
            ...
            for (int triangleIndex : triangle)
            {
                ...
                {
                    // read data
                    glm::vec3 vertexPosition = vertexPositions[vertexIndexObject.vertexPositionIdx - 1];
                    glm::vec4 textureCoord{ 0, 0, 0, 0 };
    ...
    indices.erase(std::remove_if(indices.begin(), indices.end(), [](const ObjInterleavedIndex& a) { return a.vertexIndices.empty(); }),
                  indices.end());
